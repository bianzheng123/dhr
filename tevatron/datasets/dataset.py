...
class HFTrainDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        ...
        if data_args.query_cluster_path is not None:
            data_files = {data_args.dataset_split: data_args.query_cluster_path}
            self.qidx_cluster = load_dataset(data_args.dataset_name,
                                    data_args.dataset_language,
                                    data_files=data_files, cache_dir=cache_dir)[data_args.dataset_split]
        else:
            self.qidx_cluster = None
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        return self.dataset, self.qidx_cluster
...
class HFCorpusDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        if data_args.encode_in_path is not None:
            data_files = data_args.encode_in_path
        if data_args.corpus_path is not None:
            data_files = data_args.corpus_path
        if data_files:
            data_files = {data_args.dataset_split: data_files}
        print(data_args.dataset_name, data_args.dataset_language)
        self.dataset = load_dataset(data_args.dataset_name,
                                    data_args.dataset_language,
                                    data_files=data_files, cache_dir=cache_dir)[data_args.dataset_split]
...
class HFEvalDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        ...
        self.dataset = load_dataset(data_args.dataset_name,
                                    data_args.dataset_language,
                                    data_files=data_files, cache_dir=cache_dir)[data_args.dataset_split]
        self.preprocessor = PROCESSOR_INFO[data_args.dataset_name][3] if data_args.dataset_name in PROCESSOR_INFO \
            else DEFAULT_PROCESSORS[3]
        self.tokenizer = tokenizer
        self.q_max_len = data_args.q_max_len
        self.p_max_len = data_args.p_max_len
        self.proc_num = data_args.dataset_proc_num
